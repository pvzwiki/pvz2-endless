...
test('the leader chance has a level gate and selects at most one wave', () => {
  assert.equal(
    rollLeader(3, 5, {
      bounded() {
        throw new Error('Too early to roll');
      },
    }).wave,
    null,
  );
...
test('final reservation uses earlier budget-paid appearances, independent of extra leaders', () => {
  ...
  assert.ok(
    level.waves.slice(0, -1).every((wave) => wave.paid.every((row) => row.zombie === 'mummy')),
  );
  const final = level.waves.at(-1)!;
  assert.deepEqual(
    final.reserved.map((row) => row.zombie),
    level.selection.selected.slice(1),
  );
  ...
  for (const world of ['egypt', 'eighties'] as const) {
    for (const displayed of [1, 4, 19, 21, 36, 49, 51, 69, 71, 81, 99, 101, 149]) {
      ...
      for (const wave of level.waves) {
        ...
        assert.equal(
          rosterCost(wave.instructions) - rosterCost(wave.paid),
          wave.leader ? rosterCost([wave.leader]) : 0,
        );
      ...
      if (world === 'eighties') {
        const out = applyJams(
          displayed,
          level.waves.map((wave) => wave.instructions),
          8,
        );
        assert.ok(out.events.length > 0, `music event fits level ${displayed}`);
        out.result.forEach((wave, index) => {
          assert.deepEqual(
            wave.map((row) => row.level).sort(),
            level.waves[index].instructions.map((row) => row.level).sort(),
          );
        });
      }
    }
...
test('one seed repeats a roster, while independent instructions can get either adjacent level', () => {
  ...
  assert.deepEqual(
    [...new Set(level.waves.flatMap((wave) => wave.instructions.map((row) => row.level)))].sort(),
    [4, 5],
  );
});
