...
test('a saved wave outside the shortened plan clamps to its new final wave', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Wave 8', exact: true })).toHaveAttribute(
    'aria-pressed',
    'true',
  );
  await page.getByRole('button', { name: 'Level 1', exact: true }).click();
  await expect(page.getByRole('button', { name: 'Wave 5', exact: true })).toHaveAttribute(
    'aria-pressed',
    'true',
  );
});
  ...
  for (const locale of ['en', 'zh-CN'] as const) {
    for (const chapter of articles) {
      ...
      await expect(page.getByRole('heading', { level: 1 })).toHaveText(
        (locale === 'en' ? en : zh).articles[chapter.id].title,
      );
...
test('the homepage owns the catalog and the introduction starts the reading sequence', async ({
  page,
}) => {
  await page.goto('/en/');
...
test('optional views respond to controls and return focus to the article', async ({ page }) => {
  ...
  await expect(viewer.getByRole('button', { name: 'Base budgets', exact: true })).toHaveAttribute(
    'aria-pressed',
    'true',
  );
  ...
  await expect(viewer.getByRole('button', { name: 'Wave 1', exact: true })).toHaveAttribute(
    'aria-pressed',
    'true',
  );
  await page.keyboard.press('Escape');
...
test('the matrix filters levels and opens the selected wave', async ({ page }) => {
  ...
  await viewer
    .getByRole('button', { name: 'Level 149, wave 15, 27,375 points, Final wave', exact: true })
    .click();
  await expect(page.getByTestId('current-level')).toHaveText('149');
  await viewer
    .getByRole('button', { name: 'Wave 15', exact: true })
...
test('language switching preserves the selected numerical example', async ({ page }) => {
  ...
  await page.keyboard.press('Escape');
  await page.getByRole('link', { name: '打开证据注释 01', exact: true }).click();
...
test('roster controls spend the budget and preserve the example across languages', async ({
  page,
}) => {
  ...
  expect(
    Number((await page.getByTestId('roster-remaining').textContent())!.replaceAll(',', '')),
  ).toBeLessThan(100);
...
test('the reference retains duplicate aliases and distinguishes omitted fields', async ({
  page,
}) => {
  ...
  expect(
    await page.getByRole('button', { name: 'Inspect pirate_imp', exact: true }).count(),
  ).toBeGreaterThan(1);
...
test('the plant reference preserves missing fields and bilingual search state', async ({
  page,
}) => {
  ...
  await expect(
    dialog.locator('.record-fields > div').filter({ hasText: 'type.Enabled' }).locator('dd'),
  ).toHaveText('—');
  await page.keyboard.press('Escape');
  await expect(record).toBeFocused();
...
test('mobile article and reference controls remain usable without WebGL', async ({ page }) => {
  ...
  await page.locator('.mobile-nav').getByRole('link', { name: '资料库', exact: true }).click();
  await page
    .locator('.reference-grid')
    .getByRole('link', { name: /植物资料/ })
    .click();
