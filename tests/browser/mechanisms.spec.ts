...
test('construction reuses the level-wide leader attempt when inspecting another wave', async ({
  page,
}) => {
...
test('music replay preserves sampled levels and resets when the example seed changes', async ({
  page,
}) => {
  const dialog = await open(page, 'worlds', 'jam');
  const tiles = dialog.locator('.jam-board .lab-token');
  const levels = await tiles.evaluateAll((nodes) =>
    nodes.map((node) => node.getAttribute('data-level')).sort(),
  );
  await seek(dialog, 'end');
  expect(
    await tiles.evaluateAll((nodes) => nodes.map((node) => node.getAttribute('data-level')).sort()),
  ).toEqual(levels);
  await expect(dialog.locator('.jam-board [data-replaced=true][data-leader=true]')).toHaveCount(0);
  await dialog.getByRole('button', { name: 'Next seed', exact: true }).click();
  await expect(dialog.getByRole('spinbutton', { name: 'Example seed', exact: true })).toHaveValue(
    '8',
  );
  await expect(dialog.getByRole('slider', { name: 'Step', exact: true })).toHaveValue('0');
});

test('a late portal opening retains the emission schedule and consumes one slot per update', async ({
  page,
}) => {
  const dialog = await open(page, 'worlds', 'portals');
  await dialog
    .getByRole('spinbutton', { name: 'Opening-completion time (seconds)', exact: true })
    .fill('20');
  await seek(dialog, 4);
...
test('a rejected newcomer cannot delay the zero-health automatic path', async ({ page }) => {
  ...
  await dialog
    .getByRole('checkbox', { name: 'Reject the optional newcomer before H₀', exact: true })
    .uncheck();
...
for (const locale of ['en', 'zh-CN'] as const) {
  ...
  test(`${locale}: HP saturation delays the health trigger while the timer remains active`, async ({
    page,
  }) => {
  ...
  test(`${locale}: warning-time taps retain state 2 and finalization needs another update`, async ({
    page,
  }) => {
...
test('all mechanism views and tabs work in both languages on mobile', async ({ page }) => {
  ...
  page.on('console', (message) => {
    if (
      message.type() === 'error' &&
      /MISSING_MESSAGE|INVALID_MESSAGE|Hydration|React error/.test(message.text())
    )
  ...
  for (const locale of ['en', 'zh-CN'] as const) {
    for (const [chapter, kind] of views) {
      ...
      await expect(dialog.locator('.lab-intro,.lab-tabs').first()).toBeVisible();
      expect(
        await dialog.locator('.lab-body').evaluate((el) => el.scrollWidth - el.clientWidth),
        `${locale}/${kind}`,
      ).toBeLessThan(2);
...
test('final reservation skips equality and ordinary filling can spend the same budget', async ({
  ...
  await dialog.getByRole('button', { name: 'Exact match: 200', exact: true }).click();
  await seek(dialog, 1);
  await expect(dialog.getByRole('heading', { name: 'Reservation skips this type' })).toBeVisible();
  await expect(output(dialog, 'Remaining budget')).toHaveText('200');
  await seek(dialog, 'end');
  await expect(output(dialog, 'Remaining budget')).toHaveText('0');
  await expect(dialog.locator('.reservation-result .pipeline-token')).not.toHaveCount(0);
});
...
test('an empty portal queue still blocks completion until the later close callback', async ({
  ...
  await dialog.getByRole('checkbox', { name: 'All emitted children have been cleared' }).check();
...
test('Steam applies one strict damage pass per update and preserves transport deadlines', async ({
  ...
  const dialog = await open(page, 'worlds', 'steam');
  await seek(dialog, 3);
  await dialog.getByRole('spinbutton', { name: 'Game-time delta per update' }).fill('1');
  await dialog.getByRole('button', { name: 'Run one update', exact: true }).click();
  await expect(output(dialog, 'Damage passes')).toHaveText('0');
  await dialog.getByRole('button', { name: 'Run one update', exact: true }).click();
  await expect(output(dialog, 'Damage passes')).toHaveText('1');
  await dialog.getByRole('tab', { name: 'Pipeline transport', exact: true }).click();
  ...
  await dialog.getByRole('checkbox', { name: 'Block the exit at t = 1' }).check();
  await expect(output(dialog, 'Damage rate to one blocker')).toHaveText('53');
  await dialog.getByRole('spinbutton', { name: 'Transported zombies' }).fill('5');
  await expect(output(dialog, 'Damage rate to one blocker')).toHaveText('53');
...
test('requested levels, table fallback, and leader health remain separate in the inspector', async ({
  ...
  const dialog = await open(page, 'strength', 'strength', '&strength.level=2&strength.residue=10');
  ...
  await dialog.getByRole('button', { name: 'Level-5 conehead example', exact: true }).click();
  ...
  await dialog.getByRole('button', { name: 'Inspect missing row 6', exact: true }).click();
  await expect(dialog.locator('.lab-note')).toContainText('lookup returns multiplier 1');
...
test('placement hooks retain or replace the provisional position', async ({ page }) => {
  const dialog = await open(page, 'placement', 'placement');
  await dialog.getByRole('tab', { name: 'Placement sequence', exact: true }).click();
  await dialog.getByRole('combobox', { name: 'Newcomer type' }).selectOption('0');
  await dialog.getByRole('checkbox', { name: 'An active circle accepts this newcomer' }).uncheck();
  await seek(dialog, 1);
  ...
  await dialog.getByRole('combobox', { name: 'Newcomer type' }).selectOption('1');
  await dialog.getByRole('checkbox', { name: 'An active circle accepts this newcomer' }).check();
  await dialog.getByRole('button', { name: 'All occupied', exact: true }).click();
  await seek(dialog, 4);
  await expect(dialog.getByTestId('placement-zombie')).toHaveAttribute('data-game-x', '744');
  await expect(dialog.getByTestId('placement-zombie')).toHaveAttribute('data-row', '3');
...
test('the clock distinguishes normal advancement, regular boundaries, and short final waves', async ({
  ...
  await dialog.getByRole('checkbox', { name: 'Automatic next-wave option' }).check();
  await expect(output(dialog, 'First wave advance')).toHaveText('1s');
  await dialog.getByRole('button', { name: 'Before regular flag wave 5', exact: true }).click();
  await expect(output(dialog, 'First wave advance')).toHaveText('6s');
  await dialog.getByRole('button', { name: 'Before short final wave 13', exact: true }).click();
  await expect(output(dialog, 'First wave advance')).toHaveText('1s');
  await dialog.getByRole('tab', { name: 'Complete the battle', exact: true }).click();
  await dialog
    .getByRole('button', {
    ...
    .click();
  await expect(dialog.locator('.completion-output')).toContainText('still blocked');
  await dialog
    .getByRole('button', {
    ...
    .click();
  await expect(dialog.locator('.completion-output')).toContainText('still blocked');
  await dialog.getByRole('button', { name: 'All shown predicates pass', exact: true }).click();
  await expect(dialog.locator('.completion-output')).toContainText('Completion passes');
});

test('plant-food consumption and strict saved-schedule equality are visible', async ({ page }) => {
  ...
  await dialog.getByRole('tab', { name: 'Request carriers', exact: true }).click();
  await dialog.getByRole('combobox', { name: 'Instruction order' }).selectOption('1');
  await seek(dialog, 'end');
  await expect(output(dialog, 'Quota remaining')).toHaveText('0');
  await expect(output(dialog, 'Accepted markers')).toHaveText('0');
  await dialog.getByRole('combobox', { name: 'Instruction order' }).selectOption('0');
  await seek(dialog, 'end');
  await expect(output(dialog, 'Accepted markers')).toHaveText('1');
  await expect(output(dialog, 'Pickups requested')).toHaveText('1');
  await dialog.getByRole('tab', { name: 'Advance saved schedules', exact: true }).click();
  await dialog.getByRole('button', { name: 'Inspect exact equality', exact: true }).click();
  await expect(output(dialog, 'LevelLengthsPlayed')).toHaveText('2.000');
  await expect(output(dialog, 'NextDropTime')).toHaveText('2.000');
  await expect(dialog.locator('.lab-readout h3')).toHaveText('0 loot codes emitted so far');
  await dialog.getByRole('button', { name: 'Plan another level', exact: true }).click();
  await expect(dialog.locator('.lab-readout h3')).toHaveText('1 loot codes emitted so far');
});

test('the original roster view can replay pool-index removal before filling', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Replay type selection', exact: true }).click();
  await dialog.getByRole('button', { name: 'Pause selection', exact: true }).click();
  await expect(dialog.locator('.selection-token[data-selected=true]')).toHaveCount(1);
  await dialog.getByRole('button', { name: 'Next choice', exact: false }).click();
  await expect(dialog.locator('.selection-token[data-selected=true]')).toHaveCount(2);
  await dialog.getByRole('button', { name: 'Use final selection', exact: true }).click();
  await expect(dialog.locator('.selection-token[data-selected=true]')).toHaveCount(5);
  await expect(dialog.getByRole('button', { name: 'Next draw', exact: false })).toBeVisible();
});
...
test('language switching preserves a mechanism example and its replay position', async ({
  ...
  const dialog = await open(page, 'worlds', 'jam');
  await dialog.getByRole('combobox', { name: 'Displayed level', exact: true }).selectOption('51');
  await dialog.getByRole('spinbutton', { name: 'Example seed', exact: true }).fill('19');
  await seek(dialog, 3);
  await page.keyboard.press('Escape');
  await page.getByRole('link', { name: '中文', exact: true }).click();
  await expect(page.locator('html')).toHaveAttribute('lang', 'zh-CN');
  await page.getByRole('button', { name: new RegExp(zh.labs.jam.title) }).click();
  const chinese = page.getByRole('dialog', {
    name: zh.labs.jam.title,
    exact: true,
  });
  await expect(chinese.getByRole('combobox', { name: zh.labs.level, exact: true })).toHaveValue(
    '51',
  );
  await expect(chinese.getByRole('spinbutton', { name: zh.labs.seed, exact: true })).toHaveValue(
    '19',
  );
  await expect(chinese.getByRole('slider', { name: zh.labs.step, exact: true })).toHaveValue('3');
});
...
test('wide formulas provide a localized scroll cue and keyboard access only when needed', async ({
  ...
  const formula = page.getByRole('group', { name: zh.site.scrollableFormula, exact: true }).first();
  await expect(formula).toBeVisible();
  await expect(page.locator('.math-scroll-note').first()).toContainText(zh.site.scrollFormulaHint);
  await formula.focus();
