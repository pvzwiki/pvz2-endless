...
Zero weight does not remove a record. The sampler uses `draw <= cumulative weight`, so an exact-zero draw can accept its first record even when that record is ineligible.<Note id="rowGates" />
