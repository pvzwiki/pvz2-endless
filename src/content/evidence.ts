import { equipmentEvidence } from './equipment-evidence';
export const evidence = {
  ...equipmentEvidence,
  randomStreams: {
    number: '07',
    addresses: ['0x101d47624', '0x101c79bec', '0x101ce66dc', '0x100121044'],
    en: {
      title: 'Separate random-state consumers',
      text: 'Fresh DangerRoom initialization constructs designer state, materializes waves, runs world postprocessing, installs runtime actions and loot tags, then reseeds the WaveManager. Jam uses the global RNG; Modern queue shuffling calls imported C++ machinery. The website uses reconstructed integer draws with explicit input seeds and independent scenario state.',
    },
    'zh-CN': {
      title: '独立的随机状态消耗者',
      text: '全新 DangerRoom 初始化构建设计器状态、转换波次、运行世界后处理、安装运行时动作与掉落标记，随后重新播种 WaveManager。Jam 使用全局 RNG；摩登世界队列打乱调用导入的 C++ 机制。网站使用重建的整数抽取、明确输入的种子与各情境独立的状态。',
    },
  },
  circleLifetime: {
    number: '06',
    addresses: ['0x100ccab7c', '0x100ccb0a4', '0x100cda0cc'],
    en: {
      title: 'One shared circle, refreshed rather than stacked',
      text: 'The first creation fixes the circle destination and costume auxiliary effect. A later activation reuses the shared circle and restarts its 15-second loop, preserving captured references. Cleanup clears the tracked stun condition but stores no original positions to restore. Start and end animations are separate from the timed loop.',
    },
    'zh-CN': {
      title: '共享法阵刷新，不叠加',
      text: '首次创建确定法阵目标位置与装扮附加效果。之后的激活复用共享法阵，重新开始 15 秒循环，保留被捕获对象的引用。清理会清除追踪的眩晕状态，但没有保存可供恢复的原始位置。起始与结束动画独立于计时循环。',
    },
  },
  interpolation: {
    number: '01',
    addresses: ['0x101ab6028'],
    en: {
  ...
  jam: {
    number: '02',
    addresses: ['0x101c79bec', '0x101ce76d8'],
    en: {
  ...
  portals: {
    number: '04',
    addresses: ['0x100739c58', '0x101b04448', '0x100120f80', '0x100121a00'],
    en: {
  ...
  steam: {
    number: '05',
    addresses: ['0x100b980c0', '0x101ec8840'],
    en: {
  ...
  spawnLevels: {
    number: '01',
    addresses: ['0x101d4561c', '0x101b3a27c'],
    en: {
  ...
  strengthTable: {
    number: '02',
    addresses: ['0x1019938a0', '0x101993af8', '0x101d6db00', '0x100fd8a10', '0x10067f50c'],
    en: {
      title: 'Shared table lookup and missing rows',
      text: "The non-Zomboss DangerRoom lookup uses the shared ZombieLevelStats vector at effective entity level minus one. The bundled vector has five rows and a missing row returns multiplier 1; neccd.zls can replace the ordered pairs. The adopted ten-level Endless reference is reported gameplay data. Its attack column is a reported ratio, kept separate from the native AttackLevel input. Rift and NewPVP have separate table-selection branches; NewPVP's player-facing identity is unresolved. The level-module ZombieMaxLevel default is -1, and the ordinary definitions omit an override.",
    },
    'zh-CN': {
      title: '共享表查询与缺失行',
      text: '非 Zomboss 的 DangerRoom 查询使用共享 ZombieLevelStats 向量，以实体有效等级减一作为索引。包内向量有五行，缺失行返回倍率 1；neccd.zls 可以替换有序数值对。采用的十级无尽参考来自实战反馈。其中攻击一列是反馈比例，与原生 AttackLevel 输入分开处理。Rift 与 NewPVP 有独立的选表分支；NewPVP 对应的玩家可见模式未确定。等级模块的 ZombieMaxLevel 默认为 -1，普通定义未声明覆盖值。',
    },
  },
  bite: {
    number: '03',
    addresses: ['0x1019b55b0', '0x10199f658'],
    en: {
  ...
  leaders: {
    number: '04',
    addresses: ['0x1019afe48', '0x10199a84c'],
    en: {
  ...
  positions: {
    number: '01',
    addresses: ['0x1014c40c0', '0x1014c47e8', '0x1014c544c', '0x1023aa5b8'],
    en: {
  ...
  rowGates: {
    number: '02',
    addresses: ['0x101595f3c', '0x1014fd55c', '0x101511010', '0x10156ac7c'],
    en: {
      title: 'Eligibility gates and the zero endpoint',
      text: 'The common predicate checks row count, enabled state, GridExtents.mY, and the stage gate. Pirate uses exact type-name tests; Future restricts disco_mech at the outer rows. Zero-weight records remain in the sampler. Its inclusive cumulative comparison can select the first zero-weight record at an exact-zero draw, as confirmed by a controlled native helper check.',
    },
    'zh-CN': {
      title: '资格条件与零端点',
      text: '共同谓词检查行数、启用状态、GridExtents.mY 和场景条件。海盗港湾使用精确类型名检查；未来世界限制 disco_mech 的最外侧行。零权重记录仍保留在采样器中。其包含边界的累积比较，在抽取值恰为零时可以选择第一条零权重记录，受控原生辅助函数检查证实了这一点。',
    },
  },
  rowHistory: {
    number: '03',
    addresses: ['0x10156ab20', '0x10156aba0', '0x10157f418', '0x1016246ac'],
    en: {
  ...
  circle: {
    number: '04',
    addresses: ['0x100ccab7c', '0x100cd9ea0', '0x1019958d4'],
    en: {
  ...
  specialRows: {
    number: '05',
    addresses: ['0x10238803c', '0x10151e4ec', '0x102389300', '0x101520cac'],
    en: {
  ...
  hpSaturation: {
    number: '02',
    addresses: ['0x10199dfc8', '0x102082abc', '0x101ce76d8', '0x101ce7298'],
    en: {
  ...
  waveClock: {
    number: '03',
    addresses: ['0x101ce7380', '0x101ce7ce0', '0x101ce6fe0'],
    en: {
  ...
  skipOption: {
    number: '04',
    addresses: ['0x101581d5c', '0x101ce7094', '0x101ce6e8c'],
    en: {
  ...
  rushOption: {
    number: '06',
    addresses: ['0x1015946a8', '0x1019853a8', '0x101999778'],
    en: {
  ...
  completion: {
    number: '07',
    addresses: ['0x101555514', '0x101ce4250', '0x101b5c264'],
    en: {
  ...
  portalClose: {
    number: '08',
    addresses: ['0x100122734', '0x100121a00', '0x100121e44'],
    en: {
  ...
  loss: {
    number: '09',
    addresses: ['0x10153839c', '0x1015386b4'],
    en: {
  ...
  progression: {
    number: '10',
    addresses: ['0x101ffc3e0', '0x101d6cf7c', '0x101c360dc', '0x101d4a4e8'],
    en: {
  ...
  foodPlan: {
    number: '01',
    addresses: ['0x101ab6840', '0x101ce8a54'],
    en: {
  ...
  foodMarker: {
    number: '02',
    addresses: ['0x1014c40c0', '0x1014c4324', '0x1019af834'],
    en: {
  ...
  foodDrop: {
    number: '03',
    addresses: ['0x1019a5dac', '0x10158157c', '0x101588a24'],
    en: {
  ...
  lootSchedule: {
    number: '04',
    addresses: ['0x101ce66dc', '0x101a8b5e8', '0x101a8b124'],
    en: {
  ...
  lootAssignment: {
    number: '05',
    addresses: ['0x101ce66dc', '0x1014c513c', '0x1019afe38'],
    en: {
  ...
  lootDispatch: {
    number: '06',
    addresses: ['0x101a89994', '0x1019a5dac', '0x101a8b78c'],
    en: {
  ...
  selection: {
    number: '01',
    addresses: ['0x101ab6840'],
    en: {
      title: 'Pool selection by index',
      text: 'The builder adds the stage basic type, then chooses ZombieTypeCount entries from the configured pool. Each selected vector entry is removed before the next bounded draw. The baseline count is 4. Weight is not read by this selection pass.',
    },
    'zh-CN': {
      title: '按索引选择池条目',
      text: '构建器先加入场景的基础种类，再从配置池中选择 ZombieTypeCount 个条目。每个选中的向量条目在下一次有界抽取前移除。基线数量为 4。这个选择流程不读取 Weight。',
    },
  },
  highCost: {
    number: '02',
    addresses: ['0x101ab6d78'],
    en: {
      title: 'The high-cost replacement',
      text: 'Above BigZombieSureExistLevel, baseline 50, the helper checks for a selected type whose WavePointCost is at least 1500. If absent, it draws from qualifying remaining pool entries and replaces the last pool selection. The predicate concerns cost, not a particular runtime zombie class.',
    },
    'zh-CN': {
      title: '高成本替换',
      text: '超过 BigZombieSureExistLevel（基线为 50）后，辅助函数检查已选种类中是否有 WavePointCost 至少为 1500 的类型。若没有，就从剩余池中符合条件的条目里抽取，替换最后一次池选择。这个谓词检查成本，而非某个特定的运行时僵尸类。',
    },
  },
  filling: {
    number: '03',
    addresses: ['0x101ce7f08', '0x100366650'],
    en: {
      title: 'The weighted integer bag',
      text: 'The ordinary filler drops candidates whose WavePointCost exceeds the remaining budget, draws through the integer weighted bag, appends the chosen type, and subtracts its cost. FirstWave and the separate Cost property are not inputs to these helpers. Bounded integer draws use modulo reduction; bag shares describe interval lengths, not a claim of perfectly uniform RNG residues.',
    },
    'zh-CN': {
      title: '整数权重抽签袋',
      text: '普通填充流程去掉 WavePointCost 超过剩余预算的候选，通过整数权重袋抽取，追加选中种类，并扣除其成本。FirstWave 和单独的 Cost 属性不参与这些辅助函数。有界整数抽取使用模约减；袋内份额描述区间长度，并不声称随机数余数严格均匀。',
    },
  },
  unseen: {
    number: '04',
    addresses: ['0x101ce8598'],
    en: {
      title: 'Strict final-wave reservation',
      text: 'Before normal final-wave filling, the materializer visits selected types not yet seen. A type is appended only when its cost is strictly less than the current remaining budget. The budget is reduced after each insertion, so ordering and earlier insertions affect later eligibility.',
    },
    'zh-CN': {
      title: '严格的最终波补入条件',
      text: '在普通最终波填充前，波次转换流程访问尚未出现的已选种类。只有成本严格小于当前剩余预算时才追加。每次插入都会减少预算，因此顺序和更早的插入会影响后续资格。',
    },
  },
  ordering: {
    number: '05',
    addresses: ['0x100c03c8c'],
    en: {
      title: 'Ordering and the component example',
      text: 'The native ordered-set comparator compares type-pointer values. The website uses selected display order and reconstructed integer draws. The isolated filler demonstrates budget spending; the construction and music views generate preceding waves, final reservation, per-instruction levels, and the level-wide leader attempt.',
    },
    'zh-CN': {
      title: '顺序与组件示例',
      text: '原生有序集合比较器比较类型指针值。网站使用所显示的已选顺序和重建的整数抽取。独立填充器展示预算消耗；构建与音乐视图生成此前各波、最终波预留、各指令等级，以及每关一次的首领尝试。',
    },
  },
  extras: {
    number: '06',
    addresses: ['0x101ce8a54', '0x101ab6840'],
    en: {
      title: 'Extra leader entries',
      text: 'The materializer attempts extra entries from displayed level 4 onward. The shared baseline declares LeaderProbability = 0.4 and LeaderMinWaveCost = 100. Leader eligibility uses a strict cost-greater-than test, followed by weighted selection. Successful entries are appended outside the ordinary filler budget. Wealth probability is zero in this baseline, but its chance draw is still consumed.',
    },
    'zh-CN': {
      title: '额外首领条目',
      text: '波次转换流程从显示关卡 4 起尝试额外条目。共享基线声明 LeaderProbability = 0.4、LeaderMinWaveCost = 100。首领资格使用成本严格大于阈值的检查，随后加权选择。成功条目在普通填充预算之外追加。该基线中的财神概率为零，但仍消耗其概率抽取。',
    },
  },
  shared: {
    number: '01',
    addresses: ['0x101ab6840', '0x101d6db00'],
    en: {
      title: 'The shared inputs',
      text: 'The ordinary description builder reads point fields from the shared DangerRoomPropertySheet. The response setter writes server-supplied values into that same object. The local generator declarations are a separate set of fields; they are not the point inputs consumed by this builder.',
    },
    'zh-CN': {
      title: '共享输入',
      text: '普通描述构建器读取共享 DangerRoomPropertySheet 中的点数字段。响应设置函数将服务器提供的数值写入同一个对象。生成器自身的声明属于另一组字段，不是这个构建器使用的点数输入。',
    },
  },
  level: {
    number: '02',
    addresses: ['0x101d6ca84', '0x101d4a4e8', '0x101c360dc'],
    en: {
      title: 'Which level number?',
      text: 'The designer getter returns the stored progress value plus one. The filename helper chooses the Boss variant when the displayed number is divisible by the shared BossInterval, whose bundled value is 5. The inspected post-victory continuation branch loads a next level only below 150. The explorer therefore offers ordinary numbers from 1–149; this client branch does not establish a universal server admission rule.',
    },
    'zh-CN': {
      title: '使用哪个关卡数？',
      text: '供设计器使用的读取函数返回已存进度值加一。文件名辅助函数在显示关卡数可被共享 BossInterval 整除时选择 Boss 变体，其内置值为 5。已检查的胜利后继续分支只在下一个关卡数小于 150 时加载关卡。因此交互示例提供 1–149 中的普通关卡数；这个客户端分支不能确立通用的服务器准入规则。',
    },
  },
  layout: {
    number: '03',
    addresses: ['0x101ab6840', '0x101d4a7e8'],
    en: {
      title: 'Wave count and flag spacing',
      text: 'The description builder uses MinWaveCount, MaxWaveCount, WaveAddEach, and WaveAddInterval. The inspected constructor defaults are 6, 15, 1, and 5. This guide uses the supplied gameplay wave settings 5, 15, 1, and 10. Within the resulting 5–15-wave range, the target group count is 1 below 7 waves, 2 below 12, and 3 otherwise. The resulting spacing equals the wave count divided by that target, rounded up.',
    },
    'zh-CN': {
      title: '波次数与旗帜间隔',
      text: '描述构建器使用 MinWaveCount、MaxWaveCount、WaveAddEach 和 WaveAddInterval。已检查的构造函数默认值分别是 6、15、1 和 5。本指南采用收到的实际游玩波次设置 5、15、1 和 10。在得到的 5–15 波范围内，目标分组数在少于 7 波时为 1，少于 12 波时为 2，其余为 3。得到的间隔等于波次数除以目标分组数，再向上取整。',
    },
  },
  budget: {
    number: '04',
    addresses: ['0x101ab6840', '0x101d4a7e8'],
    en: {
      title: 'The underlying budget',
      text: 'The shared constructor supplies StartingPoints = 100, BasePointIncrementPerWave = 30, and BasePointIncrementPerLevel = 5. The description builder adds the combined per-wave increment once for each zero-based wave index.',
    },
    'zh-CN': {
      title: '基础预算',
      text: '共享构造函数提供 StartingPoints = 100、BasePointIncrementPerWave = 30 和 BasePointIncrementPerLevel = 5。描述构建器根据从零开始的波次索引，逐次增加合并后的每波增量。',
    },
  },
  boost: {
    number: '05',
    addresses: ['0x101ce8a54'],
    en: {
      title: 'One boost at either boundary',
      text: 'The materialization path multiplies the base budget by 2.5 when the wave is at a regular flag boundary or is final. It applies the multiplier once even when both conditions hold. The product uses float32 arithmetic and is truncated to an integer. The next wave continues from the unboosted base progression.',
    },
    'zh-CN': {
      title: '任一边界，一次加成',
      text: '波次转换流程在常规旗帜边界波或最终波上，将基础预算乘以 2.5。即使同时满足两个条件，也只应用一次倍率。乘积使用 float32 运算，再截断为整数。下一波继续沿用未加成的基础增长规律。',
    },
  },
  scope: {
    number: '06',
    addresses: ['0x101ce7f08', '0x101c79bec', '0x101ce4f5c'],
    en: {
      title: 'What the budget pays for',
      text: 'The weighted filler consumes points using each selected type’s WavePointCost. Extra leaders are appended after normal filling; the flag zombie is installed as a separate action. Eighties can replace instructions in postprocessing without repeating the ordinary budget check. The budget therefore describes one stage of construction, not a final zombie count or total-health cap.',
    },
    'zh-CN': {
      title: '预算支付什么',
      text: '加权填充流程按所选种类的 WavePointCost 消耗点数。额外首领在普通填充完成后追加；旗帜僵尸作为独立动作安装。Eighties 可以在后处理时替换指令，而不重新进行普通预算检查。因此预算描述的是构建中的一个阶段，不是最终僵尸数量，也不是总生命值上限。',
    },
  },
} as const;
