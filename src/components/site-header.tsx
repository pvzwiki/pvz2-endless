...
export function WaveMark() {
  return (
    <svg viewBox="0 0 30 30" fill="none" aria-hidden="true">
      <path
        d="M4 23V17M11 23V10M18 23V4M25 23V13"
        stroke="currentColor"
        strokeWidth="3"
        strokeLinecap="round"
      />
      ...
      <header className="site-header">
        ...
        <nav className="desktop-nav" aria-label={locale === 'en' ? 'Main navigation' : '主导航'}>
          ...
          <Link
            href="/reference/"
            aria-current={
              ['/reference', '/plants', '/zombies', '/artifacts', '/accessories'].some((path) =>
          ...
          <Link
            href="/introduction/"
            aria-current={pathname.startsWith('/introduction') ? 'page' : undefined}
          >
            {t('about')}
          </Link>
        ...
        <div className="header-right">
          <div className="language-switch" aria-label={locale === 'en' ? 'Language' : '语言'}>
            ...
            <Link
              href={pathname}
              locale="zh-CN"
              onClick={(event) => changeLanguage(event, 'zh-CN')}
              lang="zh-CN"
              aria-label="中文"
          ...
          <details className="mobile-nav">
            <summary aria-label={locale === 'en' ? 'Open navigation' : '打开导航'}>
              <span />
              <span />
            </summary>
            <nav>
              <Link href="/articles/">{t('series')}</Link>
              <Link href="/reference/">{t('reference')}</Link>
              <Link href="/introduction/">{t('about')}</Link>
              <Link href="/search/">{t('search')}</Link>
            </nav>
          </details>
        </div>
