...
const rarities = [
  ...new Set(
    catalog.types.flatMap((row) =>
      typeof row.values['type.Rare'] === 'number' ? [row.values['type.Rare']] : [],
    ),
  ),
].sort((a, b) => a - b);
...
export function PlantReference() {
  ...
  const rows = useMemo(() => {
    ...
    return catalog.types
      .filter((row) => {
        ...
        if (rare === 'missing' ? value !== null : rare !== '' && value !== Number(rare))
          return false;
        return (
          !term ||
          [row.id, row.name.en, row.name['zh-CN'], row.framework || ''].some((text) =>
  ...
  const fieldStatus = (status: string) =>
    status === 'declared'
      ? c('declared')
      : status === 'agrees_across_declarations'
        ? t('repeated')
        : c('omittedStatus');
  const valueText = (value: PlantValue | undefined) =>
    value === null || value === undefined
      ...
      : typeof value === 'boolean'
        ? value
          ...
          : c('no')
        : typeof value === 'number'
          ? format.number(value, { maximumFractionDigits: 10 })
          : value;
  useEffect(() => {
    ...
    <>
      <section className="catalog-browser" aria-label={t('title')}>
        <div className="catalog-filters">
          ...
          <label>
            {t('rare')}
            <select value={rare} onChange={(event) => setRare(event.target.value)}>
              ...
              <option value="missing">{c('omittedStatus')}</option>
            </select>
          </label>
        </div>
        <div className="catalog-summary">
          <span aria-live="polite">{c('results', { count: rows.length })}</span>
          <p>{c('omitted')}</p>
        </div>
        ...
        <div className="catalog-table-scroll">
          <table className="catalog-table">
            <thead>
              <tr>
                <th>
                  <button onClick={() => changeSort('id')}>{c('type')}</button>
                </th>
                {coreFields.map((field, index) => (
                  ...
                  >
                    <button aria-label={c('sort', { field })} onClick={() => changeSort(field)}>
                      {labels[index]}
            ...
            <tbody>
              {rows.slice(currentPage * 40, (currentPage + 1) * 40).map((record) => (
                <tr key={record.key}>
                  <th scope="row">
                    <button
                      className="catalog-name"
                      aria-label={c('open', { name: record.name[locale] })}
                      onClick={(event) => {
                  ...
                  {coreFields.map((field) => (
                    <td key={field} title={fieldStatus(record.status[field])}>
      ...
      >
        {selected && (
          <div className="record-content">
            ...
            <h2 id="plant-title">{selected.name[locale]}</h2>
            <code className="record-identifier">{selected.id}</code>
            <div className="record-core">
              {coreFields.map((field, index) => (
                <div key={field}>
                  <span>{labels[index]}</span>
                  <strong>{valueText(selected.values[field])}</strong>
                  <code>{field}</code>
                </div>
              ))}
            </div>
            ...
            <dl className="record-fields">
              {[
                ['PlantType', selected.plantClass],
                ['PlantFramework', selected.framework],
                ['HomeWorld', selected.homeWorld],
                [c('reference'), selected.reference],
              ].map(([label, value]) => (
                <div key={label}>
                  <dt>{label}</dt>
                  <dd>{value || '—'}</dd>
            ...
            <dl className="record-fields">
              {catalog.fields
                ...
                .map((field) => (
                  <div key={field}>
                    <dt>{field}</dt>
                    <dd title={fieldStatus(selected.status[field])}>
                      {valueText(selected.values[field])}
                    </dd>
                  </div>
                ))}
            </dl>
