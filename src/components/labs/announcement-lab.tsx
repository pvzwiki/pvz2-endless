...
import { LevelControl, Stat, useCanvasWidth, useEndlessParameters } from './lab-controls';

export default function AnnouncementLab() {
  const t = useTranslations('labs.announcement'),
    locale = useLocale();
  ...
  const input = {
    ...
    intervalAfterSpawn: state.interval,
    clearAfterSpawn: state.clear,
    final: wave.final,
  };
  ...
  const end =
    Math.ceil(
      (Math.max(normal.following, selected.following, normal.clear, selected.clear) + 2) / 10,
    ) * 10;
    ...
    <div>
      ...
      <div className="lab-controls">
        ...
        <label>
          ...
          <select
            value={wave.number}
            onChange={(event) => set({ wave: Number(event.target.value) })}
          >
            {waves.map((row) => (
      ...
      <div className="lab-controls">
        ...
        <button onClick={() => set({ manual: 1, tap: (normal.visible + normal.warningStart) / 2 })}>
          {t('before')}
        </button>
        <button
          onClick={() => set({ manual: 1, tap: (normal.warningStart + normal.warningEnd) / 2 })}
        >
          {t('during')}
        </button>
      </div>
      <div className="lab-note">
        {t('window', {
          from: number(normal.warningStart),
          to: number(normal.warningEnd),
          visible: number(normal.visible),
        })}
      ...
      <div className="lab-stage" ref={ref}>
        <svg viewBox={`0 0 ${width} 247`} role="img" aria-label={t('timeline')}>
          {Array.from({ length: Math.floor(end / tickStep) + 1 }, (_, i) => i * tickStep).map(
            (time) => (
              <g key={time}>
                <line x1={x(time)} x2={x(time)} y1="28" y2="215" className="lab-axis" />
                <text x={x(time)} y="238" textAnchor="middle" className="lab-axis-label">
                  {number(time)}s
                </text>
              </g>
            ),
          ...
          {lanes.map((lane, index) => {
            ...
            return (
              <g key={index}>
                <text x="5" y={18 + y} className="lab-axis-label">
                  {t(index ? 'selected' : 'normal')}
                </text>
                {warningShown && (
                  <rect
                    x={x(lane.warningStart)}
                    y={38 + y}
                    width={Math.max(0, x(lane.spawn) - x(lane.warningStart))}
                    height="18"
                    rx="3"
                    fill="#d7b579"
                  />
                ...
                <rect
                  x={x(lane.spawn)}
                  y={68 + y}
                  width={x(lane.following) - x(lane.spawn)}
                  height="22"
                  rx="3"
                  fill={lane.retained ? '#9f8bb5' : '#6f9767'}
                />
                ...
                <circle
                  ...
                  cy={79 + y}
                  r="5"
                  fill={lane.retained ? '#d4bcdf' : '#bfd69b'}
                />
              </g>
            );
          })}
      ...
      <div className="lab-stat-grid health-report-grid" data-testid="announcement-result">
        <Stat
          label={t('spawn')}
          value={`${number(selected.spawn)}s`}
          detail={t('spawnDetail', { n: wave.number })}
        />
        <Stat
          label={t('state')}
          value={selected.stateAfterSpawn}
          detail={t(selected.retained ? 'retained' : 'timed')}
        />
        <Stat
          label={t('freshDeadline')}
          value={`${number(selected.freshDeadline)}s`}
          detail={t('intervalDetail', { n: number(state.interval) })}
        />
        <Stat
          label={t(wave.final ? 'finish' : 'following')}
          value={`${number(selected.following)}s${selected.completionNeedsAnotherUpdate ? ' +' : ''}`}
          detail={
            selected.completionNeedsAnotherUpdate
              ? t('anotherUpdate')
              : wave.final
                ? t('finishDetail')
                : t('followingDetail', { n: wave.number + 1 })
          }
        />
      </div>
      <p className="lab-note" aria-live="polite">
        {selected.retained
          ? t('retainedEffect', {
              clear: number(selected.clear),
              deadline: number(selected.freshDeadline),
            })
          : t('timedEffect')}
