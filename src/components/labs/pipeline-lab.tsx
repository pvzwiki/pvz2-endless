...
import { entityStrength, typeRecord } from '@/lib/mechanism-model';
import { LevelControl, Playback, Stat, useEndlessParameters } from './lab-controls';
const stages = [
  'inputs',
  'budget',
  'types',
...
export default function PipelineLab() {
  ...
  const generated = useMemo(
    () => generateOrdinaryRosters('egypt', level, state.seed),
    [level, state.seed],
  );
  ...
  const health = waveHealth(
    roster.map((row) => entityStrength(row.zombie, row.level, row.leader)),
  ).reported;
  return (
    <div>
      <p className="lab-intro">{t('intro')}</p>
      <div className="lab-controls">
        <LevelControl value={level} onChange={(value) => set({ level: value, wave: 1, step: 0 })} />
        <label>
          {c('wave')}
          <select
            value={wave.number}
            onChange={(event) => set({ wave: Number(event.target.value), step: 0 })}
          >
            {plan.waves.map((row) => (
      ...
      <Playback step={state.step} total={9} onChange={(step) => set({ step })} />
      ...
      <div className="lab-stat-grid">
        ...
        <Stat
          label={t('spent')}
          value={state.step >= 3 ? (wave.budget - wave.remaining).toLocaleString(locale) : '—'}
          detail={t('left', { n: wave.remaining })}
        />
        <Stat
          label={state.step < 9 ? t('count') : t('health')}
          value={
            state.step < 3 ? '—' : state.step < 9 ? roster.length : health.toLocaleString(locale)
          }
          detail={
            state.step === 9
              ? t('subtotalDetail')
      ...
      <div className="lab-stage">
        ...
        {state.step >= 6 && wave.flagType && (
          <p className="lab-note" data-testid="independent-flag">
            {t('separateFlag', { name: typeRecord(wave.flagType).name[locale] })}
          </p>
        )}
        {wave.final && state.step >= 3 && (
          <p className="lab-caption">{t('reservationDetail', { n: wave.reserved.length })}</p>
        )}
