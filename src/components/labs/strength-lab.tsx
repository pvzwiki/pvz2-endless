...
import { mechanismInputs, levelRequest, entityStrength, typeRecord } from '@/lib/mechanism-model';
    ...
    <div>
      ...
      <div className="lab-stage" ref={ref}>
        <svg viewBox={`0 0 ${width} 224`} role="img" aria-label={t('curve')}>
          {[1, 5, 10].map((n) => (
            <g key={n}>
              <line x1="35" x2={width - 18} y1={y(n)} y2={y(n)} className="lab-axis" />
          ...
          {[1, 46, 91, 149].map((n) => (
            <text key={n} x={x(n)} y="215" textAnchor="middle" className="lab-axis-label">
              {n}
            </text>
          ))}
          <polyline points={points} fill="none" stroke="#b8d596" strokeWidth="2" />
          <g
            className="lab-token"
            style={{
      ...
      <div className="lab-stat-grid">
        ...
        <Stat label={t('bounds')} value={`${request.lower} / ${request.upper}`} />
        <Stat
          label={t('requested')}
          value={chosen}
          ...
          <div className="lab-controls">
            <label>
              ...
              <select
                ...
                onChange={(event) => set({ override: Number(event.target.value) })}
              >
      ...
      <div className="lab-controls">
        <label>
          ...
          <select
            value={state.basis}
            onChange={(event) => set({ basis: Number(event.target.value) })}
          >
        ...
        <Toggle checked={!!state.leader} onChange={(value) => set({ leader: Number(value) })}>
          {t('leader')}
        </Toggle>
        <button onClick={() => set({ type: 1, override: 5, leader: 0 })}>{t('conehead')}</button>
        <button onClick={() => set({ override: 6, basis: 1 })}>{t('missing')}</button>
      </div>
      <p className="lab-caption">
        {basis === 'reported' ? t('reportedDetail') : t('bundledDetail')}
      </p>
        ...
        {[
          ...
          <div className="lab-bar-pair" key={label}>
            ...
            <div className="lab-bar">
              <span style={{ width: `${(value.body / maximum) * 100}%` }}>{value.body}</span>
              <span className="helmet" style={{ width: `${(value.helmet / maximum) * 100}%` }}>
                {value.helmet || ''}
              </span>
            </div>
          </div>
      ...
      <div className="lab-stat-grid health-report-grid">
        ...
        />
        <Stat label={t('helmet')} value={result.helmet.toLocaleString(locale)} />
        {result.bite === null ? (
          <Stat
          ...
          <tbody>
            {Array.from({ length: 10 }, (_, i) => (
              <tr key={i} className={effective === i + 1 ? 'selected' : ''}>
                ...
                <td>
                  {basis === 'reported'
                    ? mechanismInputs.reportedStrength.health[i]
                    : (mechanismInputs.strengthRows[i]?.HitPointsLevel ?? 1)}
                </td>
                <td>
                  {basis === 'reported'
                    ? mechanismInputs.reportedStrength.attack[i]
                    : (mechanismInputs.strengthRows[i]?.AttackLevel ?? 1)}
                </td>
                <td>
                  {basis === 'reported'
                    ? t('reported')
                    : i < mechanismInputs.strengthRows.length
                      ? t('declared')
                      : t('fallback')}
                </td>
              </tr>
