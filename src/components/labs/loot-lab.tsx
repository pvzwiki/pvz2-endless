...
export default function LootLab() {
  ...
  for (let run = 0; run < state.runs; run++) {
    const next = advanceLootSchedule(schedule, length, state.period, phase, state.count);
    schedule = next.state;
  ...
  const quotaStep = Math.min(state.step, 7),
    consumed = carriers.filter((row, i) => row.requested && quotaStep >= i * 3 + 2).length,
    marked = carriers.filter((row, i) => row.carrier && quotaStep >= i * 3 + 3).length;
  return (
    <div>
      ...
      {state.mode === 0 ? (
        <>
              ...
              <table className="lab-table">
                ...
                <tbody>
                  {mechanismInputs.flagRows.map((row, i) => (
                    <tr key={i} className={row === allocation.chosen ? 'selected' : ''}>
                      <td>{row.MinLevel}</td>
                      <td>{row.FlagCount}</td>
                ...
                <tbody>
                  {mechanismInputs.foodRows.map((row, i) => (
                    <tr
                      key={i}
                      className={
                        row.MinLevel <= level && row.MinPlantfoodPerFlagWave === allocation.low
                          ? 'selected'
                          : ''
                      }
                    >
          ...
          <div className="lab-stat-grid">
            ...
            <Stat label={t('bounds')} value={`${allocation.low} / ${allocation.high}`} />
            <Stat
              label={t('planned')}
        ...
        <>
          ...
          <div className="lab-controls">
            <label>
              ...
              <select
                value={state.example}
                onChange={(event) => set({ example: Number(event.target.value), step: 0 })}
              >
                {examples.map((_, i) => (
            ...
            <label>
              ...
              <input
                ...
                max={5}
                value={state.quota}
                onChange={(event) => set({ quota: Number(event.target.value), step: 0 })}
              />
            </label>
            <Toggle
          ...
          <Playback step={quotaStep} total={7} onChange={(step) => set({ step })} />
          <div className="carrier-track">
            {carriers.map((row, i) => {
              ...
              return (
                <div key={i} className={`carrier-row ${inspected ? 'active' : ''}`}>
                  <div>
                    <span>{i + 1}</span>
                  ...
                  <div className={requested ? 'active' : ''}>
                    ...
                    <strong>{requested ? (row.requested ? '−1' : t('skip')) : '—'}</strong>
                  </div>
                  <div className={accepted ? 'active' : ''}>
                    <small>{t('marker')}</small>
                    <strong>
                      {accepted ? (row.carrier ? '✓' : row.rejected ? '×' : '—') : '—'}
                    </strong>
                  </div>
                  <div className={quotaStep === 7 ? 'active' : ''}>
                    <small>{t('pickup')}</small>
                    <strong>{quotaStep === 7 && state.canDrop && row.carrier ? '●' : '—'}</strong>
                  </div>
                </div>
              );
            })}
          ...
          <div className="lab-stat-grid">
            ...
            <Stat label={t('pickups')} value={quotaStep === 7 && state.canDrop ? marked : 0} />
          </div>
            ...
            <p>
              {quotaStep === 7
                ...
                : quotaStep === 0
                  ? t('carrierStart')
                  : t('carrierStep', {
                      name: typeRecord(carriers[Math.floor((quotaStep - 1) / 3)].id).name[locale],
                      phase: t(`carrierPhases.${phaseKeys[(quotaStep - 1) % 3]}`),
                    })}
            </p>
          </div>
        </>
        ...
        <>
          ...
          <div className="lab-controls">
            ...
            <label>
              ...
              <input
                ...
                max={3}
                value={state.tags}
                onChange={(event) => set({ tags: Number(event.target.value), step: 0 })}
              />
            </label>
          </div>
          <Playback step={tagStep} total={tags.length} onChange={(step) => set({ step })} />
          ...
          <table className="lab-table">
            ...
            <tbody>
              {lootTypes.map((id, index) => (
                <tr key={id} className={tagged.includes(index) ? 'selected' : ''}>
          ...
          <div className="lab-controls">
            ...
            <label>
              ...
              <input
                ...
                step={0.001}
                value={state.length}
                onChange={(event) => set({ length: Number(event.target.value), runs: 0 })}
              />
            </label>
            <label>
              Period
            ...
            <label>
              ...
              <input
                ...
                step={0.1}
                value={phase}
                onChange={(event) => set({ phase: Number(event.target.value), runs: 0 })}
              />
            </label>
            <label>
              {t('drawnCount')}
              <input
                ...
                max={3}
                value={state.count}
                onChange={(event) => set({ count: Number(event.target.value), runs: 0 })}
              />
            </label>
          </div>
          <div className="lab-controls">
            <button onClick={() => set({ runs: state.runs + 1 })} disabled={state.runs >= 8}>
              {t('planNext')}
            </button>
            ...
            <button onClick={() => set({ entry: 0, length: 0, runs: 0 })}>
              {t('waveLength', { count: plan.count })}
            </button>
          </div>
          <div className="lab-stage" ref={ref}>
            <svg viewBox={`0 0 ${width} 150`} role="img" aria-label={t('scheduleDiagram')}>
              ...
              {Array.from(
                ...
                <g key={time}>
                  <line x1={x(time)} x2={x(time)} y1="30" y2="110" className="lab-axis" />
                  <text x={x(time)} y="131" textAnchor="middle" className="lab-axis-label">
                    {time}
                  </text>
                </g>
              ))}
              {history.flatMap((run) =>
                run.events
                  ...
                  .map((event, i) => (
                    <circle key={`${run.run}-${i}`} cx={x(event.at)} cy="75" r="6" fill="#d8b575" />
                  )),
              )}
              ...
              >
                <line x1="0" x2="0" y1="20" y2="105" stroke="#edf3d5" strokeWidth="2" />
                <path d="M-4 18 L4 18 L0 25Z" fill="#edf3d5" />
              </g>
            </svg>
          </div>
          <div className="lab-stat-grid">
            <Stat label="LevelLengthsPlayed" value={schedule.length.toFixed(3)} />
            <Stat
              label="NextDropTime"
              value={schedule.nextDrop === null ? '∞' : schedule.nextDrop.toFixed(3)}
            />
            <Stat label="NextScheduleTime" value={schedule.nextSchedule.toFixed(3)} />
          </div>
