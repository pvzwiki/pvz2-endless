...
import { LevelControl, Playback, Stat, useEndlessParameters, useCanvasWidth } from './lab-controls';
...
export default function JamLab() {
  ...
  const { generated, out } = useMemo(() => {
    ...
    const out = applyJams(
      level,
      generated.waves.map((wave) => wave.instructions),
      state.seed + 1,
    );
  ...
  const removedFinal = event.frames.filter((row) => row.kind === 'remove').at(-1)?.removed ?? [];
  const tokenState = (original: VisualInstruction) => {
    ...
    const removedIndex = removedFinal.findIndex((row) => row.key === original.key);
    ...
    const shelfIndex = frame.removed.findIndex((row) => row.key === original.key);
    return {
      ...original,
  ...
  return (
    <div>
      ...
      <div className="lab-controls">
        <LevelControl value={level} onChange={(level) => set({ level, event: 0, step: 0 })} />
        <label>
          {c('seed')}
          <input
            type="number"
            ...
            max={65535}
            value={state.seed}
            onChange={(event) => set({ seed: Number(event.target.value), event: 0, step: 0 })}
          />
        </label>
        <button onClick={() => set({ seed: (state.seed + 1) % 65536, event: 0, step: 0 })}>
          {t('newSeed')}
        </button>
      ...
      <div className="lab-wave-strip">
        {plan.waves.map((wave) => {
          const index = out.events.findIndex((item) => item.wave === wave.number),
      ...
      <div className="lab-columns">
        <div>
          <div className="lab-step-title">
            ...
            <div>
              <h3>
                {eventName(event.jam)} · {typeRecord(event.replacement_type).name[locale]}
              </h3>
              <p>
                {t('eventDetail', {
      ...
      <Playback step={step} total={event.frames.length - 1} onChange={(step) => set({ step })} />
      ...
      <div className="lab-stage jam-board" ref={canvas}>
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={t('diagram')}>
          <text x="5" y={shelfY - 3} className="lab-axis-label">
            {t('savedLevels')}
          </text>
          <line x1="5" x2={width - 10} y1={shelfY - 17} y2={shelfY - 17} className="lab-axis" />
          {before.map((original, index) => {
            ...
            return (
              ...
              >
                ...
                />
                <text x={(cell - 8) / 2} y="13" textAnchor="middle" fontSize="9" fill="#43603b">
                  {isNew ? t('new') : String(index + 1).padStart(2, '0')}
                  {item.leader ? ' ★' : ''}
                </text>
                <text x={(cell - 8) / 2} y="29" textAnchor="middle" fontSize="11" fill="#17351e">
                  {marks[item.zombie] || 'Z'}
                </text>
                <text x={(cell - 8) / 2} y="44" textAnchor="middle" fontSize="9" fill="#304b2e">
                  Lv {item.level}
                </text>
              </g>
            );
      ...
      <div className="lab-chip-list">
        {[...new Set([...before.map((row) => row.zombie), event.replacement_type])].map((id) => (
      ...
      <details className="lab-details">
        ...
        <p>
          {t('selectedTypes', {
            names: generated.types.map((type) => typeRecord(type.id).name[locale]).join(' · '),
          })}
        </p>
        <p>
          {t('budgetDetail', {
      ...
      <details className="lab-details">
        ...
        <div className="lab-chip-list">
          {out.selectionDraws.map((draw, index) => (
            <span key={index} className={`lab-chip ${draw.accepted ? '' : 'dim'}`}>
              {index + 1}. {eventName(jamNames[draw.candidate])} {draw.accepted ? '✓' : '↻'}
            </span>
          ))}
        </div>
      </details>
