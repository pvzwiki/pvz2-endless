...
import { LevelControl, Stat, Toggle, useCanvasWidth, useEndlessParameters } from './lab-controls';
const conditionKeys = [
  'finished',
  'currentClear',
  'olderClear',
...
export default function TimingLab() {
  ...
  const initialAccumulator = Math.fround(
      Math.fround(state.eligibleHp) +
    ...
    scenario = deadlineScenario(interval, crossing, !!state.automatic, guarded, large);
  const health = state.time >= state.at ? after : initial,
    deadline =
      crossing !== null && state.time >= crossing ? Math.min(interval, crossing) : interval;
  ...
  return (
    <div className="timing-lab">
      <div className="lab-tabs" role="tablist" aria-label={t('views')}>
        ...
        <button role="tab" aria-selected={state.mode === 2} onClick={() => set({ mode: 2 })}>
          {t('announcement')}
        </button>
      ...
      {state.mode === 0 ? (
        <>
          ...
          <div className="lab-controls">
            <LevelControl value={level} onChange={(level) => set({ level, wave: 1, time: 0 })} />
            <label>
              {t('trackedWave')}
              <select
                value={wave}
                onChange={(event) => set({ wave: Number(event.target.value), time: 0 })}
              >
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => set({ fraction: Number(event.target.value) })}
              />
            </label>
            <Toggle
              checked={!!state.automatic}
              onChange={(checked) => set({ automatic: Number(checked), time: 0 })}
            >
              {t('automatic')}
            </Toggle>
            <Toggle
          ...
          <div className="lab-controls">
            <label>
              ...
              <input
                ...
                value={state.damage}
                onChange={(event) => set({ damage: Number(event.target.value) })}
              />
            </label>
            <label>
              {t('lossAt')}
          ...
          {initialAccumulator > INT32_MAX && (
            <p className="lab-note" data-testid="health-saturation">
              {t('saturationDetail', { cap: INT32_MAX.toLocaleString(locale) })}
            </p>
          )}
          <div className="lab-stage" ref={ref}>
            <svg viewBox={`0 0 ${width} 235`} role="img" aria-label={t('timeline')}>
              {[0, 10, 20, 30, 40].map((time) => (
                <g key={time}>
                  <line x1={x(time)} x2={x(time)} y1="28" y2="207" className="lab-axis" />
                  <text x={x(time)} y="225" textAnchor="middle" className="lab-axis-label">
                    {time}s
                  </text>
                </g>
              ))}
              ...
              />
              <g className="lab-token" style={{ transform: `translate(${x(state.time)}px,0)` }}>
                <line x1="0" x2="0" y1="30" y2="204" stroke="#eef0d9" strokeWidth="2" />
          ...
          <div className="lab-stat-grid">
            <Stat
              ...
              value={`${deadline}s`}
              detail={
                large ? t('largeInterval') : t('ordinaryInterval', { n: state.normalInterval })
              }
            />
            <Stat
              label={t('visibility')}
            ...
            <Stat
              ...
              detail={
                scenario.request ? t('manualPath') : large ? t('withAnnouncement') : t('normalPath')
              }
            />
          </div>
        ...
        <>
          ...
          <div className={`completion-output ${win ? 'pass' : ''}`} aria-live="polite">
            <span>AND</span>
