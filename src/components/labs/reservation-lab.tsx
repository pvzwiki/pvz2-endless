...
import { Playback, Stat, Toggle, useEndlessParameters } from './lab-controls';
const ids = ['mummy', 'mummy_armor1', 'mummy_armor2', 'tomb_raiser', 'egypt_gargantuar_danger'];
export default function ReservationLab() {
  ...
  const currentPass = step > 0 && step <= passes.length ? passes[step - 1] : null;
  const fillStep = step > passes.length ? filler.steps[step - passes.length - 1] : null;
  const seenPasses = passes.slice(0, Math.min(step, passes.length)),
    visible = [
      ...seenPasses.filter((row) => row.accepted).map((row) => row.id),
      ...filler.steps.slice(0, Math.max(0, step - passes.length)).map((row) => row.chosen.id),
    ];
  ...
  return (
    <div>
      ...
      <div className="lab-controls">
        <label>
          ...
          <input
            ...
            max={30000}
            value={state.budget}
            onChange={(event) => set({ budget: Number(event.target.value), step: 0 })}
          />
        </label>
        <label>
          {c('seed')}
        ...
        <button onClick={() => set({ budget: 200, unseen: 2, step: 0 })}>{t('exact')}</button>
        <button onClick={() => set({ budget: 600, unseen: 6, step: 0 })}>{t('orderCase')}</button>
      </div>
        ...
        <table className="lab-table">
          ...
          <tbody>
            {types.map((type, index) => (
              <tr key={type.id} className={currentPass?.id === type.id ? 'selected' : ''}>
                ...
                <td>
                  <Toggle
                    ...
                    onChange={(value) =>
                      set({
                        unseen: value ? state.unseen | (1 << index) : state.unseen & ~(1 << index),
                        step: 0,
                      })
                    }
                  >
      ...
      <div className="lab-stat-grid">
        <Stat
          label={t('phase')}
          value={step === 0 ? t('ready') : step <= passes.length ? t('reservation') : t('filler')}
        />
