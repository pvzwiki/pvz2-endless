...
export default function PortalLab() {
  ...
  const frames = useMemo(
    () => portalReplay(portal.types, state.shufflePosition, offset, state.interval, state.opening),
    [portal, state.shufflePosition, offset, state.interval, state.opening],
  );
  const step = Math.min(state.step, frames.length - 1),
    frame = frames[step],
    current = frame.state;
  const queue = [...current.children, ...current.queue];
  const emitted = current.children.length,
    remaining = current.queue.length;
  const alpha = clamp(f32(f32(state.level - 1) / 29), 0, 1),
    range = (a: [number, number], b: [number, number], shift = 0) => {
      const values = [
        ...new Set(
          a.flatMap((first) =>
            b.map((second) => Math.trunc(f32(first + (second - first) * alpha)) + shift),
          ),
        ),
      ].sort((x, y) => x - y);
    ...
    <div>
      ...
      <div className="lab-controls">
        ...
        <label>
          ...
          <select
            value={state.family}
            onChange={(event) => set({ family: Number(event.target.value), step: 0 })}
          >
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => set({ shufflePosition: Number(event.target.value), step: 0 })}
          />
        </label>
        <button
          onClick={() => set({ shufflePosition: (state.shufflePosition + 1) % 65536, step: 0 })}
        >
          {t('reshuffle')}
        </button>
        <label>
          ...
          <input
            ...
            onChange={(event) => set({ interval: Number(event.target.value), step: 0 })}
          />
        </label>
        <label>
          {t('offset')}
          <input
            ...
            max={state.interval}
            value={offset}
            onChange={(event) => set({ offset: Number(event.target.value), step: 0 })}
          />
        </label>
        <label>
          {t('opening')}
      ...
      <Playback step={step} total={frames.length - 1} onChange={(step) => set({ step })} />
      ...
      <div className="lab-stat-grid">
        ...
        <Stat
          label={t('clock')}
          value={current.phase === 'removed' ? t('callbackTime') : `${current.time}s`}
        />
      </div>
          ...
          {portal.types.map((id, slot) => {
            ...
            return (
              ...
              >
                ...
                <rect width={cell - 10} height="50" rx="6" fill={isOut ? '#d3b179' : '#b2caa1'} />
                <text x={(cell - 10) / 2} y="19" textAnchor="middle" fontSize="11" fill="#30472a">
                  {t('slot', { n: slot + 1 })}
                </text>
                <text x={(cell - 10) / 2} y="37" textAnchor="middle" fontSize="9" fill="#30472a">
      ...
      <Toggle checked={!!state.cleared} onChange={(checked) => set({ cleared: Number(checked) })}>
        {t('clearChildren')}
      </Toggle>
