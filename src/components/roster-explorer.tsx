...
const color = (id: string) => palette[egypt.types.findIndex((type) => type.id === id)];
const mark = (id: string) => marks[egypt.types.findIndex((type) => type.id === id)];
...
function Experiment() {
  ...
  const selection = useMemo(() => selectTypes(plan.level, seed), [plan.level, seed]);
  const types = selection.selected.map((id) => egypt.types.find((type) => type.id === id)!);
  const result = useMemo(
    () =>
      fillBudget(
        budget,
        selection.selected.map((id) => egypt.types.find((type) => type.id === id)!),
        seed + 1,
      ),
    [budget, selection],
  );
  const step = result.steps[Math.max(0, Math.min(cursor - 1, result.steps.length - 1))];
  const remaining =
    cursor === 0
      ? budget
      : (result.steps[Math.min(cursor - 1, result.steps.length - 1)]?.after ?? budget);
  const name = (id: string) => egypt.types.find((type) => type.id === id)!.name[locale];
  useEffect(() => {
    const query = new URLSearchParams(window.location.search);
    const savedSeed = Number(query.get('seed')),
      savedBudget = Number(query.get('budget'));
    if (query.has('seed') && Number.isInteger(savedSeed) && savedSeed >= 0 && savedSeed <= 65535)
      setSeed(savedSeed);
    if (
      query.has('budget') &&
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setSelectionStep((value) => value + 1), 950);
    return () => window.clearTimeout(timer);
  }, [selectionPlaying, selectionStep]);
  const displayedTypes =
    selectionStep === 5 ? selection.selected : selection.initial.slice(0, selectionStep + 1);
  const remainingPool =
    selectionStep === 5
  ...
  const selectionDraw =
    selectionStep > 0 && selectionStep < 5 ? selection.draws[selectionStep - 1] : null;
  const columns = Math.max(3, Math.min(8, Math.floor((selectionWidth - 10) / 94)));
  const cell = (selectionWidth - 10) / columns;
  const poolY = 43 + Math.ceil(5 / columns) * 76;
  const poolRows = Math.ceil((selectionStep === 5 ? remainingPool.length : 8) / columns);
  ...
  return (
    <div className="roster-experiment">
      <div className="roster-controls">
        <label>
          {t('level')}
          <select value={plan.level} onChange={(event) => setLevel(Number(event.target.value))}>
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => {
              ...
              if (Number.isInteger(value) && value >= 0 && value <= 65535) setSeed(value);
            }}
          />
        </label>
        <button className="new-selection" onClick={() => setSeed((value) => (value + 1) % 65536)}>
          {t('reselect')} ↻
        </button>
        <label>
          {t('budget')} B
          <input
            ...
            onChange={(event) => {
              const value = Number(event.target.value);
              if (Number.isInteger(value) && value >= 0 && value <= 30000) setBudget(value);
            }}
          />
        </label>
      </div>
      <section className="type-selection">
        ...
        <div className="selection-playback">
          ...
          {selectionPlaying && (
            <button onClick={() => setSelectionPlaying(false)}>{t('pauseSelection')}</button>
          )}
          <button
        ...
        <div className="selection-stage" ref={selectionCanvas}>
          ...
          >
            ...
            {egypt.types.map((type) => {
              ...
              const index = chosen ? selectedIndex : remainingPool.indexOf(type.id);
              const x = 5 + (index % columns) * cell,
                y = 27 + Math.floor(index / columns) * 76 + (chosen ? 0 : poolY);
              return (
                <g
                  key={type.id}
                ...
                >
                  ...
                  <text x="9" y="15" className="selection-index">
                    {type.id === egypt.basic ? 'b' : chosen ? `#${selectedIndex}` : `j=${index}`}
                  </text>
                  <text
                    x={(cell - 10) / 2}
                    y="32"
                  ...
                  <text x={(cell - 10) / 2} y="49" textAnchor="middle" className="selection-cost">
                    c = {type.cost}
                  </text>
                </g>
              );
          ...
          <div className="draw-layout">
            <div className="weight-instrument">
              ...
              <div className="weight-bag">
                {types.map((type) => {
                  const candidate = step?.candidates.find((item) => item.type.id === type.id);
            ...
            <div className="draw-readout">
              ...
              <output data-testid="roster-remaining">{format.number(remaining)}</output>
              <div className="draw-result" aria-live="polite">
                <span>{cursor ? t('drawNumber', { number: cursor }) : t('ready')}</span>
                ...
                {cursor > 0 && step && <small>d = {format.number(step.draw)}</small>}
              </div>
              <button
                className="draw-next"
              ...
              <div className="draw-transport">
                <button
                  disabled={!result.steps.length || cursor >= result.steps.length}
          ...
          <div className="roster-sequence" aria-label={t('resultSequence')}>
            {result.steps.slice(0, cursor).map((entry, index) => (
              <span
                ...
                style={{ '--type-color': color(entry.chosen.id) } as CSSProperties}
              >
  ...
  return (
    ...
    >
      <div className="viewer-header">
        ...
        <button className="close-view" aria-label={t('close')} onClick={() => setView(null)}>
          ×
        </button>
      </div>
