...
export function queryCatalog(
  ...
  const rows = catalog.types.filter((record) => {
    if (scope === 'endless' && !Object.keys(record.roles).some((id) => catalog.worlds.includes(id)))
      return false;
    if (world && !record.roles[world]) return false;
    return (
      !term ||
      [record.id, record.name.en, record.name['zh-CN'], record.zombieClass || ''].some((value) =>
