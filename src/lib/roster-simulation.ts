...
export function generateOrdinaryRosters(
  ...
  const instruction = (zombie: string, leader = false): Instruction => ({
    ...
    level:
      request.lower === request.upper
        ? request.lower
        : rng.bounded(100) < request.threshold
          ? request.upper
          : request.lower,
  });
  ...
  const waves = plan.waves.map((wave) => {
    ...
    const reservation = wave.final
      ? reserveUnseen(wave.budget, unseenBefore)
    ...
    const leaderType =
      wave.number === leaderAttempt.wave
        ? drawWeighted(
            types.filter((type) => type.cost > mechanismInputs.leaderMinCost),
            rng,
          )?.chosen
        : undefined;
    ...
    return {
      ...wave,
      unseenBefore: unseenBefore.map((type) => type.id),
      reserved,
      filled,
      paid,
