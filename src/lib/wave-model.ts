...
export const ordinaryLevels = Array.from({ length: 149 }, (_, i) => i + 1).filter(
  (level) => level % baseline.bossInterval !== 0,
);
...
export function createWavePlan(level: number): WavePlan {
  ...
  const count = Math.max(
    settings.MinWaveCount,
    Math.min(
      settings.MaxWaveCount,
  ...
  const increment =
    settings.BasePointIncrementPerWave + settings.BasePointIncrementPerLevel * level;
  const waves = Array.from({ length: count }, (_, index): Wave => {
    ...
    const budget =
      flag || final ? Math.trunc(Math.fround(Math.fround(base) * baseline.flagMultiplier)) : base;
